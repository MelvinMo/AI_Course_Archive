...
class Puzzle:
    ...
    def euclidean(self, start, goal):
        """ Calculates the Euclidean distance heuristic between the start and goal nodes """
        distance = 0
                ...
                if start[i][j] != goal[i][j] and start[i][j] != '_':
                    ...
                    for a in range(self.n):
                        for b in range(self.n):
                            if start[i][j] == goal[a][b] and start[i][j] != '_':
                                goal_i, goal_j = a, b
                                break
    ...
    def manhattan(self, start, goal):
        """ Calculates the Manhattan distance heuristic between the start and goal nodes """
        distance = 0
        for i in range(self.n):
            for j in range(self.n):
                if start[i][j] != goal[i][j] and start[i][j] != '_':
...
typ = input("Enter 0 for Manhattan and 1 for Euclidean \n")
puz = Puzzle(3,typ)
puz.process()